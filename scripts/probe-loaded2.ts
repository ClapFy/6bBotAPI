...
bot._client.on("tab_complete", (p: unknown) => console.log("TAB_IN", JSON.stringify(p).slice(0, 300)));
...
bot.once("spawn", async () => {
  try {
    ...
    console.log("forced player_loaded");
    await sleep(600);

    bot._client.write("tab_complete", { transactionId: 1, text: "/ski" });
    const tabWait = Date.now() + 4000;
    while (Date.now() < tabWait) await sleep(200);
    ...
    while (Date.now() < until) {
      ...
      if (gm === "survival") {
        console.log("LEFT LOBBY", fmt(), gm);
        return;
      }
      await sleep(250);
    }
    console.log("still lobby", fmt(), bot.game.gameMode, "health", bot.health);
  } catch (error) {
    console.error(error);
  } finally {
    bot.quit("probe");
